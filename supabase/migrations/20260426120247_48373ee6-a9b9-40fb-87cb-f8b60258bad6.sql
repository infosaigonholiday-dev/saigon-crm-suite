...
BEGIN
  ...
  -- App-level key (os_v2_app_*) và Legacy key đều dùng "Basic <token>".
  -- Chỉ Organization-level key (os_v2_org_*) mới dùng "Key <token>".
  ...
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO public.push_send_log (notification_id, user_id, title, error)
    VALUES (NEW.id, NEW.user_id, NEW.title, SQLERRM);
  END;
