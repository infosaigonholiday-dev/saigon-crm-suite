...
import { Loader2, Plus, ShieldCheck, ShieldOff, Settings as SettingsIcon } from "lucide-react";
...
export default function Settings() {
  ...
  async function checkAdminAndLoad() {
    ...
    if (admin) {
      await Promise.all([loadProfiles(), loadDepartments()]);
    }
    setLoading(false);
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.success(`Tạo tài khoản thành công. Mật khẩu tạm: ${data.temp_password}`);
      setDialogOpen(false);
  ...
  async function handleToggleActive(profile: Profile) {
    ...
    try {
      ...
      toast.success(
        profile.is_active ? "Đã vô hiệu hóa tài khoản" : "Đã kích hoạt tài khoản"
      );
      await loadProfiles();
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {!isAdmin ? (
        <p className="text-muted-foreground">
          Bạn không có quyền truy cập phần quản lý tài khoản.
        </p>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setDialogOpen(true)}>
              <Plus className="h-4 w-4 mr-1" /> Thêm nhân viên mới
            </Button>
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Tạo tài khoản đăng nhập cho nhân viên. Mật khẩu tạm sẽ được tạo tự động.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
